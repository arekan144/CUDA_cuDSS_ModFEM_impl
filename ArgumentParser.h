...
matrix type -m (default "general") [(g)eneral, (s)ymmetric, (h)ermitian, symmetric-positive-defined (spd),
hermitian-positive-defined (hpd)] evaluation -e (default yes) [(y)es, (n)o, (l)oad solution] solution file -s
*/
  ...
  private:
    void parse()
    {
        ...
        if (inputs['m'].first == "g" || inputs['m'].first == "gen" || inputs['m'].first == "general")
            ...
            inputs['m'].second = 2;
        else if (inputs['m'].first == "spd" || inputs['m'].first == "symmetric-pd" ||
                 inputs['m'].first == "symmetric-positive-definite")
            inputs['m'].second = 3;
        else if (inputs['m'].first == "hpd" || inputs['m'].first == "hermitian-pd" ||
                 inputs['m'].first == "hermitian-positive-defined")
            inputs['m'].second = 4;
        else
            throw std::string("Error: Wrong matrix type.");

        if (inputs['t'].first == "e" || inputs['t'].first == "E" || inputs['t'].first == "eigen" ||
            inputs['t'].first == "eigen+cudss")
            inputs['t'].second = 0;
        else if (inputs['t'].first == "c" || inputs['t'].first == "C" || inputs['t'].first == "cusolve" ||
                 inputs['t'].first == "cusolve+cudss")
            inputs['t'].second = 1;
    ...
    std::map<const char, std::pair<std::string, unsigned short>> inputs{
        ...
        {'m', {std::string("g"), 0}},  // matrix type -m (default "general") [(g)eneral, (s)ymmetric, (h)ermitian,
                                       // symmetric-positive-defined (spd), hermitian-positive-defined (hpd)]
